...
fs = 70000;                  % ADC 有效取樣頻率，單位 Hz
lowFreqMax = 500;            % 低頻頻譜顯示上限，單位 Hz
wideFreqMax = 5000;          % 寬頻頻譜顯示上限，單位 Hz
maxTimePlotSamples = 10000;  % 時域圖最多顯示點數
numberOfPeaks = 10;          % 顯示前幾個主要頻率峰值
peakSearchRange = [1, 500];  % 峰值搜尋範圍，單位 Hz
targetFrequencies = [60, 120, 180, 240];
targetSearchRange = 5;       % 搜尋指定頻率時使用 +/-5 Hz
...
measurementTime = numberOfSamples / fs;
...
figure('Name', 'ADC time-domain signals', 'Color', 'w');
...
%% ==================== 0~500 Hz 線性頻譜 ====================
figure('Name', 'Low-frequency FFT', 'Color', 'w');
tiledlayout(3, 1, 'TileSpacing', 'compact', 'Padding', 'compact');
...
%% ==================== 0~5 kHz dB 頻譜 ====================
figure('Name', 'Wide-frequency FFT', 'Color', 'w');
tiledlayout(3, 1, 'TileSpacing', 'compact', 'Padding', 'compact');

nexttile;
plotSpectrumDb(iSenFreq, iSenAmplitudeDb, wideFreqMax, 'i\_sen FFT spectrum');
nexttile;
plotSpectrumDb(vbusFreq, vbusAmplitudeDb, wideFreqMax, 'Vbus FFT spectrum');
nexttile;
plotSpectrumDb(vacFreq, vacAmplitudeDb, wideFreqMax, 'Vac FFT spectrum');

%% ==================== 三通道低頻比較 ====================
figure('Name', 'ADC low-frequency spectrum comparison', 'Color', 'w');
...
legend('i\_sen', 'Vbus', 'Vac', 'Location', 'best');
...
function columnIndex = findColumnByKeyword(dataTable, keywords)
    ...
    if isempty(columnIndex)
        fprintf('\n找不到指定欄位。CSV 欄位如下：\n');
        disp(variableNames.');
        error('請修改 findColumnByKeyword 的關鍵字。');
    end
end
